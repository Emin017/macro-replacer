...
NEW_MACRO_DEF_FILE = "test_macro.v" # This should be the file containing the NEW macro definition
...
def main():
    parser = argparse.ArgumentParser(description="Replace macro instantiation in Verilog file.")
    parser.add_argument("--verilog", default=VERILOG_FILE, help="Top level Verilog file")
    parser.add_argument("--module", default=TARGET_MODULE, help="Module containing the macro to replace")
    parser.add_argument("--old-macro", default=OLD_MACRO_NAME, help="Name of the macro to replace")
    parser.add_argument("--new-macro-file", required=True, help="File containing new macro definition")
    parser.add_argument("--new-macro-name", required=True, help="Name of the new macro module")
    ...
    if not target_instance:
        print(f"Error: Instance of macro '{old_macro}' not found in module '{target_module}'.")
        # Try checking if it's in the instances list at top level (unlikely for blackbox internal)
        return

    print(f"Found instance '{target_instance['instanceName']}' at offsets {target_instance['startOffset']}-{target_instance['endOffset']}")
    ...
    for conn in target_instance.get("connections", []):
         ...
         signal = conn.get("signalType") # This holds the expression string now, e.g. "clk_i" or "dat_i[63:0]"
         if port and signal:
    ...
    if not new_macro_ports:
        print(f"Failed to get ports for new macro '{new_macro_name}' from '{new_macro_file}'.")
    ...
    def clean(s): return s.lower().replace("_", "")
    ...
    for new_port in new_macro_ports:
        ...
        for old_port, signal in existing_connections.items():
            ...
            # Common renames
            if clean(old_port) == "clk" and "clk" in clean(np_name): best_old_port = old_port
            if clean(old_port) == "cw" and clean(np_name) == "wen": best_old_port = old_port

            # Map BWEB (new) to BWEN (old) or BWEB (old)
            if "bweb" in clean(np_name) and clean(old_port) in ["bwen", "bweb"]: best_old_port = old_port
            # ... add more heuristics as needed

        # Special casing for the specific macro if needed
        if not best_old_port and np_name == "CLK": best_old_port = "CLK"
    ...
    if preceding_word == old_macro:
        ...
        print(f"Warning: Preceding word '{preceding_word}' does not match old macro '{old_macro}'. Replacement might Result in invalid syntax.")
